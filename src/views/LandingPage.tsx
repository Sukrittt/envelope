...
const FAQS = [
  { q: 'Why give every rupee a job?', a: 'Your bank balance includes money for rent, groceries, future plans, and fun. Assigning it to envelopes shows which money is available for each purpose before you spend. Savings is a purpose too: you do not have to spend everything you assign.' },
  { q: 'Does ₹0 ready to assign mean I’m out of money?', a: 'No. It means you have given all your available money a purpose. The money is still yours until you spend it. Check each envelope’s available balance to see what remains for that purpose.' },
  ...
  { q: 'Is Aviary free?', a: 'Aviary is free to use and open source. You can inspect the code on GitHub. You do not need a payment card to get started.' },
  { q: 'Do I connect my bank? How do expenses get added?', a: 'There is no bank connection. You log purchases yourself, which makes spending a deliberate check-in. Recurring expenses and receipt scanning help with repeated or longer entries. Transactions are not automatically imported from your bank.' },
  { q: 'Can I use it on my phone and computer?', a: 'Yes. Android and web share your account’s budgets and transactions. Android also offers a home-screen widget and notifications. New expenses can be logged offline on mobile and sync when you reconnect; other actions need a connection. There is no published iPhone app; use the web version on iPhone.' },
  { q: 'What happens to my financial data?', a: <>Sensitive financial fields are encrypted in storage. The server decrypts them to run the app; this is not end-to-end encryption. Money Brain and AI briefs send relevant transaction and budget context to Google Gemini, and bill scanning sends receipt photos. You can export or delete your data in account settings. <Link href="/legal/privacy">Read the privacy policy</Link> for storage, analytics, and processing details.</> },
  { q: 'Can I use another currency?', a: 'Aviary is built for Indian rupees, with ₹ amounts and Indian number grouping. It is not a multi-currency budgeting tool.' },
]
...
export function LandingPage() {
  return <LandingMotion><div className="lp" id="top">
    ...
    <header className="lp-header"><div className="lp-header-inner">
      <a href="#top" className="lp-logo" aria-label="Aviary home">Aviary<span aria-hidden="true">.</span></a>
    ...
    <section className="lp-opening" aria-labelledby="landing-title">
      <div><h1 id="landing-title">Give every rupee<br />a <span>job to do.</span></h1><p>Rent money. Chai money. Someday money.<br className="lp-desktop-break" /> Aviary gives each rupee a job, so you know what’s available before you spend.</p></div>
      <div className="lp-opening-actions"><a className="lp-button lp-button--accent" href={PLAY_STORE}><Smartphone size={18} />Get it on Android</a><Link className="lp-button lp-button--outline" href="/expense"><Monitor size={18} />Open web app</Link><span>Made for rupees. Free &amp; open source.</span></div>
    </section>
