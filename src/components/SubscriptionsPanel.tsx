...
const SUFFIX: Record<string, string> = { yearly: '/yr', quarterly: '/qtr', weekly: '/wk' }

function daysTo(date: string): number {
  return Math.round((new Date(date).getTime() - new Date(new Date().toISOString().slice(0, 10)).getTime()) / 86400000)
}
...
export function SubscriptionsPanel({ active, cancelled, hideAmounts, busyService, onAdd, onEdit, onCancel, onReactivate }: Props) {
  const { formatCurrency } = useCurrency()
  const [showCancelled, setShowCancelled] = useState(false)
  const money = (v: number) => (hideAmounts ? '---' : formatCurrency(Math.round(v)))
  ...
  for (const sub of sorted) {
    ...
    if (days >= 0 && (!next || days < next.days)) next = { service: sub.service, days }
  }
  ...
  function row(sub: Subscription, i: number, isActive: boolean) {
    ...
    const meta = capitalize(
      isActive ? [cycle, due ? `Next ${formatDateShort(due)}` : ''].filter(Boolean).join(' · ') : sub.renewalOrEndMonth || 'n/a',
    )
    ...
    const secondary = [
      ...
      isActive && totalRaw > 0 ? `${((monthlyEq(sub) / totalRaw) * 100).toFixed(0)}%` : '',
    ]
      .filter(Boolean)
      .join(' · ')
    ...
    return (
      <li key={sub.service} className={`subp-row ${isActive ? '' : 'is-cancelled'}`}>
        <button type="button" className="subp-row-main" onClick={() => onEdit(sub)} title="Edit subscription">
          <span className="subp-tile" style={{ color, background: `color-mix(in oklab, ${color} 22%, transparent)` }}>
          ...
          <span className="subp-amount">
            <strong>{money(showMonthly ? monthlyEq(sub) : sub.amountInr)}</strong>
            {secondary && <span>{secondary}</span>}
          </span>
        </button>
        {busyService === sub.service ? (
          <span className="subp-busy">{isActive ? 'Cancelling…' : 'Reactivating…'}</span>
        ) : (
          <button
            type="button"
            className="subp-action"
            onClick={() => (isActive ? onCancel(sub.service) : onReactivate(sub.service))}
          >
  ...
  return (
    <article className="erd-card erd-subs-panel">
      <div className="erd-panel-head">
        <h3>Subscriptions</h3>
        <button type="button" className="erd-manage-btn" onClick={onAdd} title="Add subscription">
          + Add
        </button>
      </div>
