...
const candidate = {
  fingerprint: 'x', currency: 'INR', payments: [],
  timing: {
    sortedDates: ['2026-08-04', '2026-09-03'],
    intervals: [{ days: 30, calendarMonths: 1, billingDayDrift: 1 }],
    deterministicCadence: 'monthly' as const,
  },
}
const caller = { userId: 'u', feature: 'suggest' as const }
const answer = (pattern = 'subscription', probability = 0.95) => ({ usage: {}, answers: { pattern: { choice: pattern, probabilities: { [pattern]: probability } } } })
const withCadence = (deterministicCadence: 'daily' | 'weekly' | 'monthly' | 'quarterly' | 'yearly' | 'uncertain') => ({
  ...candidate,
  timing: { ...candidate.timing, deterministicCadence },
})
beforeEach(() => vi.clearAllMocks())
it('accepts supported confident decisions and logs usage', async () => {
  evaluate.mockResolvedValue(answer('subscription', 0.81))
...
it('accepts quarterly cadence only for subscriptions', async () => {
  evaluate.mockResolvedValue(answer('subscription'))
  expect(await evaluateRecurring(withCadence('quarterly'), caller)).toEqual({ pattern: 'subscription', frequency: 'quarterly' })
  evaluate.mockResolvedValue(answer('other_recurring'))
  expect(await evaluateRecurring(withCadence('quarterly'), caller)).toBeNull()
})
it('abstains for repeat purchases, uncertain cadence, or low probability', async () => {
  for (const [value, input] of [[answer('repeat_purchase'), candidate], [answer('subscription'), withCadence('uncertain')], [answer('subscription', 0.7), candidate]] as const) {
    evaluate.mockResolvedValue(value)
    expect(await evaluateRecurring(input, caller)).toBeNull()
  }
})
it('does not turn missing probabilities or failed calls into cacheable negative results', async () => {
  evaluate.mockResolvedValue({ usage: {}, answers: { pattern: { choice: 'subscription' } } })
...
it('bounds the complete UTF-8 state and question payload, including hostile long text', () => {
  ...
  const timing = {
    ...candidate.timing,
    sortedDates: payments.map(payment => payment.date),
    intervals: Array.from({ length: 35 }, () => ({ days: 30, calendarMonths: 1, billingDayDrift: 0 })),
  }
  const payload = buildRecurringEvaluation({ ...candidate, payments, timing })
  ...
  expect(payload.state.timing.sortedDates).toHaveLength(12)
  expect(payload.state.timing.intervals).toHaveLength(11)
  expect(payload.questions.pattern).toBeDefined()
  expect(payload.questions).not.toHaveProperty('cadence')
  expect(payload.state.payments[0]).not.toHaveProperty('id')
  expect(() => buildRecurringEvaluation({ ...candidate, payments, timing }, 100)).toThrow()
})

it('sends deterministic timing as evidence and asks Jev only about the relationship', () => {
  ...
  expect(payload.state.timing).toEqual(candidate.timing)
  expect(payload.questions.pattern.instructions).toContain('Classify the payment relationship, not its cadence')
  expect(payload.questions).not.toHaveProperty('cadence')
})
