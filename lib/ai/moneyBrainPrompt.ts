...
export function buildSystemPrompt(facts: string, currencyCode: string = 'INR', decision = false): string {
  return [
    ...
    '',
    'ENVELOPE RULES: every envelope starts each month at its assignment; unspent money does not carry over, so an envelope\'s available balance is this month\'s only, including savings envelopes. READY TO ASSIGN is income no envelope has claimed; it comes back every month unless assignments change, so it is the first place to find room for a new recurring cost. Treat an emergency fund as the last resort, never the plan for a want. Use LEFT THIS MONTH for how much is left, and FREE FOR NEW COSTS for whether something new fits; do not rebuild either from envelope rows. A per-day allowance is the unspent money in spending envelopes (not savings envelopes) divided by days left. For an overspent envelope, the fix is spending less, not a lower assignment: cutting the assignment only makes the overspend bigger.',
    '',
